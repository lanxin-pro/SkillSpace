...
import { getJob,getJobNextTimes } from '@/api/infra/job/index.js'
...
const open = async (id) => {
  ...
  if (id) {
    ...
    try {
      const responseJob = await getJob(id)
      detailData.value = responseJob.data
      // 获取下一次执行时间
      const responseTime = await getJobNextTimes(id)
      nextTimes.value = responseTime.data
    } finally {
