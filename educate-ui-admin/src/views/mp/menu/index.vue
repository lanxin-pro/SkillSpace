...
<script setup>
import { getMenuList,saveMenu,deleteMenu } from '@/api/mp/menu/index.js'
...
const activeIndex = ref(MENU_NOT_SELECTED)
...
const getList = async () => {
  loading.value = false
  try {
    const response = await getMenuList(accountId.value)
    const menuData = menuListToFrontend(response.data)
...
const menuClicked = (parent, x) => {
  ...
  // 表示一级菜单
  tempSelfObj.value.grand = Level.Parent
  // 表示一级菜单索引
  tempSelfObj.value.x = x
  isParent.value = true
...
// 二级菜单点击事件
const subMenuClicked = (child, x, y) => {
  // 右侧的表单相关
  // 右边菜单
  showRightPanel.value = true
  ...
  // 表示二级菜单
  tempSelfObj.value.grand = Level.Child
  // 表示一级菜单索引
  tempSelfObj.value.x = x
  // 表示二级菜单索引
  tempSelfObj.value.y = y
...
const onDeleteMenu = async () => {
  try {
    await ELComponent.confirm('确定要删除吗?')
...
const onSave = async () => {
  try {
    await ELComponent.confirm('确定要保存吗?')
    loading.value = true
...
const onClear = async () => {
  try {
    ...
    handleQuery()
    ELComponent.notifySuccess('清空成功')
  } finally {
    loading.value = false
  }
}
