...
const emit = defineEmits(["update:modelValue","menu-clicked","submenu-clicked"])
...
const addSubMenu = (i, parent) => {
  // 获取二级菜单key长度
  const subMenuKeyLength = parent.children.length
  const addButton = {
    name: '子菜单名称',
    reply: {
      // 用于存储回复内容
...
const onParentDragEnd = ({ oldIndex, newIndex }) => {
  ...
  let positions = new Array(menuList.value.length).fill(false)
  positions[props.parentIndex] = true
  // 移出菜单，保存到变量out
  const [out] = positions.splice(oldIndex, 1)
  // 把out变量插入被移出的菜单
  positions.splice(newIndex, 0, out)
