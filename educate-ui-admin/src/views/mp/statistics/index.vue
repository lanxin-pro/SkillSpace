...
import {
  ...
  getUserSummary,
  getUserCumulate
} from '@/api/mp/statistics/index.js'
import Pagination from '@/components/Pagination/index.vue'
import { dateFormatter,formatDate,beginOfDay,endOfDay,betweenDay,addTime } from '@/utils/formatTime'
...
const initUserCumulateChart = async () => {
  ...
  // 发起请求
  try {
    const response = await getUserCumulate({
      accountId: accountId.value,
      date: [formatDate(dateRange.value[0]), formatDate(dateRange.value[1])]
    })
...
const initUpstreamMessageChart = async () => {
  ...
  // 发起请求
  try {
    const response = await getUpstreamMessage({
      accountId: accountId.value,
      date: [formatDate(dateRange.value[0]), formatDate(dateRange.value[1])]
    })
...
const interfaceSummaryChart = async () => {
  ...
  // 发起请求
  try {
    const response = await getInterfaceSummary({
      accountId: accountId.value,
      date: [formatDate(dateRange.value[0]), formatDate(dateRange.value[1])]
    })
