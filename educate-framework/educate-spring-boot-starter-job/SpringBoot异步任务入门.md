...
在 [「2.5 异步调用测试」](https://www.iocoder.cn/Spring-Boot/Async-Job/?yudao#) 中，两个方法只是发布异步调用，并未执行完成。在一些业务场景中，我们希望达到异步调用的效果，同时主线程**阻塞等待**异步调用的结果。
...
- 和 [「2.1 引入依赖」](https://www.iocoder.cn/Spring-Boot/Async-Job/?yudao#) 一致。
