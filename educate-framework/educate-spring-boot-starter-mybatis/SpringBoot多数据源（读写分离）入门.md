...
- [1. 概述](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- \2. 实现方式
    - [2.1 方案一](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
    - [2.2 方案二](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
    - [2.3 方案三](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- [3. baomidou 多数据源](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- [4. baomidou 读写分离](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- [5. MyBatis 多数据源](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- [6. Spring Data JPA 多数据源](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- [7. JdbcTemplate 多数据源](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- [8. Sharding-JDBC 多数据源](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- [9. Sharding-JDBC 读写分离](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
- [666. 彩蛋](http://www.iocoder.cn/Spring-Boot/dynamic-datasource/)
...
不过呢，这种方式在结合 Spring 事务的时候，会存在无法切换数据源的问题。具体我们在 [「3. baomidou 多数据源」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 中，结合示例一起来看。

艿艿目前找了一圈开源的项目，发现比较好的是 baomidou 提供的 [`dynamic-datasource-spring-boot-starter`](https://mp.baomidou.com/guide/dynamic-datasource.html) 。所以我们在 [「3. baomidou 多数据源」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 和 [「4. baomidou 读写分离」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 中，会使用到它。
...
咳咳咳，是不是又处于懵逼状态了？！没事，咱在 [「5. MyBatis 多数据源」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#)、[「6. Spring Data JPA 多数据源」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#)、[「7. JdbcTemplate 多数据源」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 中，结合案例一起看。「Talk is cheap. Show me the code」
...
在 [「8. Sharding-JDBC 多数据源」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#)、[「9. Sharding-JDBC 读写分离」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 中，我们会演示这种方案。
...
- spring.datasource.dynamic配置项，设置 dynamic-datasource-spring-boot-starter动态数据源的配置内容。
...
- `@DS`注解，是dynamic-datasource-spring-boot-starter提供，可添加在 Service 或 Mapper 的类/接口上，或者方法上。在其value属性中，填写数据源的名字。
...
- 我们仔细看看#method052()方法，我们添加的@Transactionl注解，使用的事务传播级别Propagation.REQUIRES_NEW

  。此时，在执行#method052()方法之前，TransactionInterceptor 会将原事务挂起，暂时性的将原事务信息和当前线程解绑。
...
和 [「3.1 引入依赖」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 一致。

## 4.2 Application

和 [「3.2 Application」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 一致。
...
- 在dynamic-datasource-spring-boot-starter中，多个相同角色的数据源可以形成一个数据源组。判断标准是，数据源名以下划线_分隔后的首部即为组名。例如说，slave_1和slave_2形成了slave组。
...
和 [「3.4 MyBatis 配置文件」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 一致。
...
- 我们并没有添加 `@MapperScan` 注解，为什么呢?答案我们在 [「5.5 配置类」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 上看。
...
和 [「3.5 实体类」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 一致。

## 5.7 Mapper

和 [「3.6 Mapper」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 基本一致，差别在于分出了 `orders` 和 `users` 两个。具体看如下两个传送门：
...
- 每个测试场景，和 [「3.8 详细测试」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 的测试场景是相对应的，按照编号。
...
> 艿艿：整个过程，和 [「5. MyBatis 多数据源」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 是类似的，所以讲解会想对精简一些。
>
> 内心 OS ：就是想偷懒，嘿嘿。
...
和 [「3.5 实体类」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 基本一致，差别在于增加了 JPA 相关注解。具体看如下两个传送门：
...
和 [「3.6 Mapper」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 基本一致，差别在于使用 Spring Data Repository 接口。具体看如下两个传送门：
...
和 [「5.8 简单测试」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 基本一致，具体看如下两个传送门：
...
和 [「5.9 详细测试」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 基本一致，具体看如下两个传送门：
...
和 [「5.10 读写分离」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) **思路**基本一致。
...
下面，我们来使用 Sharding-JDBC 来实现多数据源。整个的示例，我们会和 [「2. baomidou 多数据源」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 是一样的功能，方便胖友做类比。
...
和 [「3.6 Mapper」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 一致。

## 8.7 简单测试

和 [「3.7 简单测试」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 一致。

## 8.8 详细测试

和 [「3.8 详细测试」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 代码一致，**结果略有差异**。

在 [「3.8 详细测试」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 的场景二 `#method02()` 的测试，它会抛出异常。而对于本小节使用 Sharding-JDBC 的情况下，正常跑通。这是为什么呢？

原因实际在 [「2.3 方案三」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 已经解释了：**分库分表中间件返回的 Connection 返回的实际是动态的 DynamicRoutingConnection ，它管理了整个请求（逻辑）过程中，使用的所有的 Connection ，而最终执行 SQL 的时候，DynamicRoutingConnection 会解析 SQL ，获得表对应的真正的 Connection 执行 SQL 操作**。
...
下面，我们来使用 Sharding-JDBC 来实现读写分离。整个的示例，我们会和 [「3. baomidou 读写分离」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 是一样的功能，方便胖友做类比。

## 9.1 引入依赖

和 [「8.1 引入依赖」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 一致。

## 9.2 Application

和 [「8.2 Application」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 一致。
...
和 [「4.6 OrderMapper」](https://www.iocoder.cn/Spring-Boot/dynamic-datasource/?yudao#) 基本一致，差别是无需 `@DS` 注解，具体看如下两个传送门：
