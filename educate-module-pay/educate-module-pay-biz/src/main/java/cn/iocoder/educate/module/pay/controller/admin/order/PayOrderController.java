package cn.iocoder.educate.module.pay.controller.admin.order;
...
import cn.iocoder.educate.module.pay.controller.admin.order.vo.*;
import cn.iocoder.educate.module.pay.convert.order.PayOrderConvert;
...
import cn.iocoder.educate.module.pay.framwork.pay.core.WalletPayClient;
import cn.iocoder.educate.module.pay.service.app.PayAppService;
import cn.iocoder.educate.module.pay.service.order.PayOrderService;
...
public class PayOrderController {
    ...
    @PreAuthorize("@lanxin.hasPermission('pay:order:query')")
    ...
    public CommonResult<PayOrderDetailsRespVO> getOrderDetail(@RequestParam("id") Long id) {
        ...
        // 拼接返回
        PayAppDO app = payAppService.getApp(order.getAppId());
        PayOrderExtensionDO orderExtension = payOrderService.getOrderExtension(order.getExtensionId());
    ...
    public CommonResult<PayOrderSubmitRespVO> submitPayOrder(@RequestBody PayOrderSubmitReqVO reqVO) {
        ...
        if (Objects.equals(reqVO.getChannelCode(), PayChannelEnum.WALLET.getCode())) {
            ...
            channelExtras.put(WalletPayClient.USER_TYPE_KEY, String.valueOf(WebFrameworkUtils.getLoginUserType()));
    ...
    public CommonResult<PageResult<PayOrderPageItemRespVO>> getOrderPage(@Valid PayOrderPageReqVO pageVO) {
        ...
        List<Long> collectAppId = pageResult.getList().stream()
                .map(PayOrderDO::getAppId)
                .collect(Collectors.toList());
        // 拼接返回
        Map<Long, PayAppDO> appMap = payAppService.getAppMap(collectAppId);
        return success(PayOrderConvert.INSTANCE.convertPage(pageResult, appMap));
    }

    @GetMapping("/export-excel")
    @Operation(summary = "导出支付订单 Excel")
    @PreAuthorize("@lanxin.hasPermission('pay:order:export')")
    ...
    public void exportOrderExcel(@Valid PayOrderExportReqVO exportReqVO,
        ...
        // 拼接返回
        Map<Long, PayAppDO> appMap = payAppService.getAppMap(list.stream()
                .map(PayOrderDO::getAppId).collect(Collectors.toList()));
