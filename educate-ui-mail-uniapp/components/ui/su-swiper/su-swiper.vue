...
<template>
  <view>
    <view class="ui-swiper" :class="[this.mode, this.bg]">
      <swiper
          :circular="this.circular"
          :current="state.cur"
          :autoplay="this.autoplay && !this.state.videoPlaySataus"
      ...
      <template v-if="!state.videoPlaySataus">
        <view class="ui-swiper-dot" :class="this.dotStyle" v-if="this.dotStyle == 'tag'">
          <view
              class="ui-tag radius-lg"
              :class="[this.dotCur]"
              style="pointer-events: none; padding: 0 10rpx"
          >
            <view style="transform: scale(0.7)">{{ state.cur + 1 }} / {{ this.list.length }}</view>
          </view>
        </view>
      </template>
    </view>
...
export default {
  ...
  computed: {
    customStyle() {
      ...
      if (this.height === 0) {
        ...
        } else if (this.seizeHeight !== 0) {
          height = this.seizeHeight
        }
      }
  ...
  methods: {
    ...
    animationfinish() {
      // Implement animationfinish logic
      this.state.moveX = 0
    },
    onSwiperItem(item) {
      // Implement onSwiperItem logic
    },
    onImgLoad() {
      // Implement onImgLoad logic
    },
    ...
    swiperChange(e) {
      ...
      this.state.cur = e.detail.current
      this.state.videoPlaySataus = false
    },
    videoTimeupdate() {
      // Implement videoTimeupdate logic
    }
  }
};
</script>
