...
public class CourseSectionController {
    ...
    @Operation(summary = "/查询章节管理列表信息")
    @PreAuthorize("@lanxin.hasPermission('course:section:save')")
    public CommonResult<CourseSectionRespVO> saveUpdateChapterLesson(@RequestBody CourseSectionReqVO courseSectionReqVO) {
        CourseSectionDO courseSectionDO = courseSectionService.saveUpdateChapterLesson(courseSectionReqVO);
        return success(CourseSectionConvert.INSTANCE.convert(courseSectionDO));
    }

    @GetMapping("/get")
    @Operation(summary = "/查询章节管理列表信息")
    @PreAuthorize("@lanxin.hasPermission('course:section:get')")
    public CommonResult<CourseSectionRespVO> getChapterLessons(@RequestParam("opid") Long opid) {
        // 本章信息
        CourseSectionDO chapterLessons = courseSectionService.getChapterLessons(opid);
