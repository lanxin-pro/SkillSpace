...
public class MpUserController {
    ...
    @PreAuthorize("@lanxin.hasPermission('mp:user:query')")
    ...
    public CommonResult<MpUserRespVO> getUser(@RequestParam("id") Long id) {
        MpUserDO user = mpUserService.getUser(id);
        return success(MpUserConvert.INSTANCE.convert(user));
    }

    @PutMapping("/update")
    @Operation(summary = "更新公众号粉丝")
    @PreAuthorize("@lanxin.hasPermission('mp:user:update')")
    ...
    @PreAuthorize("@lanxin.hasPermission('mp:user:sync')")
