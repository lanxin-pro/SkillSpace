...
import cn.iocoder.educate.module.mp.convert.tag.MpTagConvert;
...
import cn.iocoder.educate.module.mp.dal.mysql.tag.MpTagMapper;
...
public class MpTagServiceImpl implements MpTagService{
    ...
    public void deleteTag(Long id) {
        ...
        try {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.TAG_DELETE_FAIL,
    ...
    public List<MpTagDO> getTagList() {
        return mpTagMapper.selectList(new LambdaQueryWrapper<>());
    }
    ...
    public void syncTag(Long accountId) {
        ...
        List<MpTagDO> mpTagDOS = mpTagMapper.selectListByAccountId(accountId);
        Map<Long, MpTagDO> tagMap = mpTagDOS.stream()
                .collect(Collectors.toMap(MpTagDO::getTagId, Function.identity(),(v1,v2)->v1));
        ...
        if (CollUtil.isNotEmpty(tagMap)) {
            List<Long> tagList = tagMap.values()
                    .stream()
                    .map(MpTagDO::getId)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList());
            mpTagMapper.deleteBatchIds(tagList);
        }
